...
//LAs unicas partes que toqué de este archivo están comentadas, se hizo pra almacenar los mensajes recibidos
'use strict';
...
const { insertarMensaje } = require('../../../../modules/funcionesImportantes');
...
const addAnswer$3 =
    ...
    (answer, options, cb = null, nested = []) => {
        ...
        const ctxAnswer = () => {
            ...
            /* console.log("Esto tiene lastctx XD: ") */
            return {
                ...lastCtx,
                ref,
                answer,
...
let CoreClass$1 = class CoreClass {
    ...
    handleMsg = async (messageCtxInComming) => {
        //Esta función ocurre si es que se recibe un mensaje

        //Para propositos de almacenamiento de datos, la variable usuario contiene el nombre del usuario, en caso de no tener será registrado con su numero en la DataBase (Firebase)
        var mensaje_entrante=messageCtxInComming
        var usuario
        if (mensaje_entrante.nombre){
            usuario=mensaje_entrante.nombre
        }else{
            usuario= mensaje_entrante.from
        }
        //Almacenamos el mensaje en la DB
        insertarMensaje(usuario,mensaje_entrante.body,mensaje_entrante.fecha,mensaje_entrante.timestamp,"cliente")

        const { body, from, fecha, nombre,timestamp} = messageCtxInComming;
        let msgToSend = [];
    ...
    sendProviderAndSave = async (numberOrId, ctxMessage) => {
        ...
        if (answer && answer.length && answer !== '__call_action__') {
            ...
            /* console.log(`TU servidor es Meta pibe, y vos quisiste enviar un mensaje a ${numberOrId} con el mensaje ${answer} y los datos ${JSON.stringify(ctxMessage)} <-- Ahí termina`) */
